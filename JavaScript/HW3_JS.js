...
let unique = Array.from(new Set(users_info.map(item => JSON.stringify(item)))).map(item => JSON.parse(item))
console.log(unique)
// - если нужно вернуть формат в json
...
const enterprises = [
  ...
  function buildTree(tree) {
    ...
    tree.forEach(function(e) {
        let summa = 0
        if (e.departments){
            e.departments.forEach(function(i){
                if(i.name == i.name)
                summa = summa + i.employees_count
            })
        }   
        if(summa == 0){
            summa = 'нет'
        }
        result += e.name + ' (' + summa + ' сотрудников)' +  '\n'
        if (e.departments){
            e.departments.forEach(function(i){
                if(i.employees_count > 0){
                result += '- ' + i.name + ' (' + i.employees_count + ' сотрудников)' + '\n'
                }
                else{
                result += '- ' + i.name + ' (нет сотрудников)' + '\n'    
            }})
        }     
    })
return result;
