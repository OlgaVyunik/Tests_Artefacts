...
WITH ranked
    ...
    AS (SELECT ranq, COUNT(*) AS competitors
        ...
        GROUP BY ranq)
SELECT b_name, b_quantity
FROM ranked JOIN counted USING (ranq)
...
FROM (
    ...
    WHERE (sb_finish <= CURDATE() AND sb_is_active = 'Y')
		OR (sb_finish > CURDATE() AND sb_is_active = 'N')
	) AS diffs;
...
SELECT AVG(DATEDIFF(CURDATE(), sb_start)) AS avg_days
FROM subscriptions;
...
SELECT b_name AS book, GROUP_CONCAT(a_name ORDER BY a_name separator ', ') AS 'author(s)'
FROM books
	JOIN m2m_books_authors using(b_id)
    JOIN authors using(a_id)
GROUP BY b_id
...
SELECT b_name AS book, 
	GROUP_CONCAT(DISTINCT a_name ORDER BY a_name separator ', ') AS 'author(s)',
...
WHERE b_id IN (SELECT DISTINCT b_id
                ...
                WHERE g_id IN (SELECT g_id
								FROM genres
                                WHERE g_name IN (N'Programming', N'Classic')
                                ))
ORDER BY b_name ASC;
...
SELECT b_id, b_name, COUNT(a_id) AS authors_count
FROM books
JOIN m2m_books_authors USING (b_id)
GROUP BY b_id
...
WITH books_taken AS
    ...
    GROUP BY sb_book)
SELECT b_id, b_name,
( b_quantity - IFNULL((SELECT taken
						FROM books_taken
...
WITH books_taken
    ...
    GROUP BY sb_book)
SELECT b_id, b_name,
	(b_quantity - IFNULL(taken, 0)) AS real_count
FROM books
LEFT OUTER JOIN books_taken ON b_id = sb_book
ORDER BY real_count DESC;
...
FROM subscriptions
WHERE sb_is_active = 'Y' AND sb_finish < current_date()
)
...
JOIN m2m_books_authors USING (a_id)
LEFT OUTER JOIN subscriptions ON m2m_books_authors.b_id = sb_book
GROUP BY a_id) AS prepared_data;

/* 57. Show median authors popularity */
WITH authors_popularity
...
SELECT EXISTS (SELECT b_id
	FROM books
    LEFT OUTER JOIN (SELECT sb_book, COUNT(sb_book) AS taken
					FROM subscriptions
                    WHERE sb_is_active = 'Y'
                    GROUP BY sb_book
                    ) AS books_taken
				ON b_id = sb_book
	WHERE (b_quantity - IFNULL(taken, 0)) < 0
    LIMIT 1)
AS error_exists;
...
SELECT g_id, g_name, AVG(popularity) AS avg_genre
FROM (
SELECT g_id, g_name, COUNT(b_id) AS popularity
FROM genres
JOIN m2m_books_genres USING (g_id)
...
AS (SELECT g_id, g_name, COUNT(b_id) AS popularity,
	ROW_NUMBER() OVER (ORDER BY COUNT(b_id) ASC) AS `row_number`
    FROM genres
JOIN m2m_books_genres USING (g_id)
JOIN books USING (b_id)
LEFT OUTER JOIN subscriptions ON b_id = sb_book
GROUP BY g_id)
SELECT g_name, AVG(popularity) AS med_genre
FROM genres_popularity,
	(SELECT COUNT(*) AS `row_count`
    FROM genres_popularity) AS total_rows
WHERE `row_number` IN (FLOOR ((`row_count` + 1)/2),
						FLOOR ((`row_count`+ 2)/2));
...
FROM (SELECT s_id, s_name, b_name, g_id,  COUNT(g_id) AS genres_count
	FROM subscribers
...
FROM (SELECT DISTINCT b_id, g_id, sb_book, sb_subscriber
	FROM books
    JOIN m2m_books_genres USING (b_id)
    JOIN subscriptions ON b_id = sb_book
     ) AS prepared_data
...
SELECT DISTINCT s_id, s_name, DATEDIFF(sb_finish, sb_start) as days
FROM subscribers
JOIN subscriptions ON s_id = sb_subscriber
WHERE sb_is_active = 'N'
AND DATEDIFF(sb_finish, sb_start) = 
	(SELECT DATEDIFF(sb_finish, sb_start) as days
    FROM subscriptions
    WHERE sb_is_active = 'N'
    ORDER BY days ASC
    LIMIT 1);
...
WITH prepared_data
AS (SELECT DISTINCT s_id, s_name, DATEDIFF(sb_finish, sb_start) as days
	FROM subscribers
    JOIN subscriptions ON s_id = sb_subscriber
    WHERE sb_is_active = 'N')
...
WITH prepared_data
AS (SELECT DISTINCT s_id, s_name, DATEDIFF(sb_finish, sb_start) as days,
	RANK() OVER (ORDER BY DATEDIFF(sb_finish, sb_start) ASC) as `rank`
    FROM subscribers
    JOIN subscriptions ON s_id = sb_subscriber
    WHERE sb_is_active = 'N')
SELECT s_id, s_name, days
...
SELECT s_id, s_name,
	GROUP_CONCAT(b_name ORDER BY b_name SEPARATOR ', ') AS books_list
FROM (SELECT s_id, s_name, b_name
	FROM subscribers
    JOIN (SELECT subscriptions.sb_subscriber,
				subscriptions.sb_book
			FROM subscriptions
            JOIN (SELECT sb_subscriber, MIN(sb_start) AS min_date
				FROM subscriptions
                GROUP BY sb_subscriber)
                AS first_visit
			ON subscriptions.sb_subscriber = first_visit.sb_subscriber
                AND subscriptions.sb_start = first_visit.min_date)
			AS books_list
		ON s_id = sb_subscriber
        JOIN books ON sb_book = b_id) AS prepared_data
GROUP BY s_id;
...
WITH t 
	AS (SELECT sb_subscriber
		FROM subscriptions
    ORDER BY sb_start DESC LIMIT 1)
SELECT s_name
FROM subscribers
...
WITH prepared_data
AS (SELECT DISTINCT s_id, s_name, DATEDIFF(sb_finish, sb_start) as days,
	RANK() OVER (ORDER BY DATEDIFF(sb_finish, sb_start) DESC) as `rank`
    FROM subscribers
    JOIN subscriptions ON s_id = sb_subscriber
    WHERE sb_is_active = 'N')
SELECT s_id, s_name, days
...
SET sb_finish = CURDATE(), /* или указать дату явно '2021-04-09'*/
	sb_is_active = 'N'
WHERE sb_id = 99; /* Обязательно указывать условие, иначе будут обновлены все поля таблицы */

/* 81. For all books that were taken on Jan 25, 2016 by the subscriber with id 2 prolong promised return date for two months */
UPDATE subscriptions
SET sb_finish = DATE_ADD(sb_finish, INTERVAL 2 MONTH)
...
INSERT INTO library.genres
	(g_id, g_name)
SELECT g_id, g_name
FROM library_exp.genres
ON duplicate key update /* to simulate MERGE in MySQL */
library.genres = CONCAT(library.genres.g_name, '[OLD]');
